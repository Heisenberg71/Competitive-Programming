#include <bits/stdc++.h>
#define ll long long
#define LIM 1000006
using namespace std;

char charStr[LIM], charPatt[LIM];
string str, patt;

ll prefHash[2][LIM], basePower[2][LIM];
ll mod[2] = {1000000007, 1000000009};
ll base[2] = {31, 101};

void preCal()
{
    prefHash[0][0] = prefHash[1][0] = str[0] - 'a' + 1;
    basePower[0][0] = basePower[1][0] = 1;

   for(int j = 0; j < 2; j++){
        for(int i = 1; i < str.size(); i++){
            basePower[j][i] = (basePower[j][i - 1]*base[j]) %mod[j];
            prefHash[j][i] = (prefHash[j][i - 1]*base[j] + str[i] - 'a' + 1) %mod[j];
        }
    }
}

ll evaluateHash(int idx, int L, int R)
{
    if(L == 0) return prefHash[idx][R];
    return (prefHash[idx][R] - (prefHash[idx][L - 1]*basePower[idx][R - L + 1]) %mod[idx] + mod[idx]) %mod[idx];
}

ll getHash(int idx)
{
    ll ans = 0;
    for(int i = 0; i < patt.size(); i++){
        ans = (ans*base[idx] + patt[i] - 'a' + 1) %mod[idx];
    }
    return ans;
}

int main()
{
    int t, cas = 1;
    scanf("%d", &t);
    while(t--){
        scanf("%s %s", charStr, charPatt);
        str = charStr, patt = charPatt;

        preCal();
        ll pattHash = getHash(0);
        ll pattHash2 = getHash(1);

        int ans = 0;
        for(int i = 0, j = patt.size() - 1; j < str.size(); i++, j++){
            if(pattHash == evaluateHash(0, i, j) && pattHash2 == evaluateHash(1, i, j)) ans++;
        }

        printf("Case %d: %d\n", cas++, ans);
    }

    return 0;
}
